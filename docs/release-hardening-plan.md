...
| H10 | Must fix | `status <run-id>`, `report <run-id>`, explicit `continue`, and `close` now exist; `cancel` remains latest-run only and failures can still surface rough HTTP/Docker errors | Remaining commands accept a run ID where meaningful, expose `--json` for automation, and print concise remedies and stable exit codes |
